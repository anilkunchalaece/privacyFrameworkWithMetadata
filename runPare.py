...
sys.path.append('.')
from lib.pare_orig.core.tester import PARETester
from lib.pare_orig.utils.demo_utils import (
...
def main(args):
    ...
    if demo_mode == 'video':
        ...
        output_path = args.output_folder #os.path.join(args.output_folder, "orig")
        os.makedirs(output_path, exist_ok=True)

        output_img_folder = os.path.join(output_path, 'pare_results')
        os.makedirs(output_img_folder, exist_ok=True)

        output_img_folder_custom_shape = os.path.join(output_path, 'customShape')
